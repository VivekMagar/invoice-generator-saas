...
// Create client safely - may fail if env vars are not set, but that's ok for display
...
// Fallback to a dummy client if initialization failed
if (!supabase) {
  supabase = {
    auth: {
...
try {
  ...
  console.warn('Supabase admin client initialization failed:', err)
}
